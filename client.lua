...
timer = 0;
anns = {};
RegisterNetEvent('Deadssentials:Announce')
AddEventHandler('Deadssentials:Announce', function(msg)
	timer = 0;
	announcement = true;
	ann = msg;
	if #ann > 70 then 
...
if Config.ReviveSystem.enable then
	...
	Citizen.CreateThread(function()
		while true do 
			...
			if IsEntityDead(ped) and not deadCheck then
				deadCheck = true;
				TriggerServerEvent("Deadssentials:DeathTrigger");
			else 
	...
	RegisterNetEvent('Deadssentials:RevivePlayer')
	AddEventHandler('Deadssentials:RevivePlayer', function()
		local ped = GetPlayerPed(-1);
		if IsEntityDead(ped) then 
			revivePed(ped);
	...
	RegisterNetEvent('Deadssentials:RespawnPlayer')
	AddEventHandler('Deadssentials:RespawnPlayer', function()
		local ped = GetPlayerPed(-1);
		if IsEntityDead(ped) then
...
currentTime = "0:00";
RegisterNetEvent('Deadssentials:SetTime')
AddEventHandler('Deadssentials:SetTime', function(time)
	currentTime = time;
end)
currentDay = 1;
RegisterNetEvent('Deadssentials:SetDay')
AddEventHandler('Deadssentials:SetDay', function(day)
	currentDay = day;
end)
currentMonth = 1;
RegisterNetEvent('Deadssentials:SetMonth')
AddEventHandler('Deadssentials:SetMonth', function(month)
	currentMonth = month;
end)
currentYear = "2021";
RegisterNetEvent('Deadssentials:SetYear')
AddEventHandler('Deadssentials:SetYear', function(year)
	currentYear = year;
end)
peacetime = false;
...
RegisterNetEvent('Deadssentials:SetAOP')
AddEventHandler('Deadssentials:SetAOP', function(aop)
	currentAOP = aop;
end)
RegisterNetEvent('Deadssentials:SetPT')
AddEventHandler('Deadssentials:SetPT', function(pt)
	peacetime = pt;
end)
